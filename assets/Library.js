...
function parseNums(num1pre, num2pre, mode) {
	...
	var num1 = num1pre, num2 = num2pre, skip=false, stringMode1=true, stringMode2=true, neg=[false, false, false], decimal=0, decimal1=0, decimal2=0, num1pos, num2pos, maxChar, numl;
    ...
    for (var i=0; !skip && num2.length==maxChar && i<num2.length && (((neg[1]||neg[2])&&mode==1)||mode==2); i++) if (+num2[i] > +num1[i]) neg[0] = true, skip = true;
	...
	if (decimal1 != decimal2 && [1,2].indexOf(mode)>-1) {
		...
		else if (decimal2 == decimal) for (var i=0;i<decimal2-decimal1;i++) num1.push("0");
	}
	if (num1.length!=num2.length && [1,2].indexOf(mode)>-1) {
		...
		else if (maxChar!=num1[0]) for (var i=0; i<numl[1]-numl[0]; i++) num1.unshift("0");
	}
...
function roundUp(num) {
	return add(num.split(".")[0], {num:["1"],isNeg:false,decimals:0});
}
function multi() {
	function tempmulti(num1pre, num2pre) {
		var parsedNums = parseNums(num1pre, num2pre, 3), neg = [parsedNums.isNeg, parsedNums.num1.isNeg, parsedNums.num2.isNeg], final = "", decimals=parsedNums.decimals, numArray = [], num2 = parsedNums.num2, num1 = parsedNums.num1;
		if (num2.num.length==1 && num2.num[0] == "1") return formatNums(num2.num, decimals, neg);
        else if (num2.length==1 && num2[0]=="0") return "1";
        else {
        	final = add(num1, num1);
        	for (var i="2"; isLessThan(i, num2); i=add({num:i.split(""),isNeg:false,decimals:0}, {num:["1"],isNeg:false,decimals:0})) final = add(final, num1);
		}
...
function expo() {
	function tempexpo(num1pre, num2pre) {
		var parsedNums = parseNums(num1pre, num2pre, 5), num1 = parsedNums.num1, num2 = parsedNums.num2, decimals = parsedNums.decimals, decimal2 = parsedNums.num2.decimals, neg = [parsedNums.isNeg, parsedNums.num1.isNeg, parsedNums.num2.isNeg], numArray=[], final="";
		if (neg[1]) num1.num = "-"+num1.num;
		if (neg[2]) num2.num = "-"+num2.num;
		if (decimal2>0) {
        	...
        	if (num2.num.length==1 && num2.num[0]=="1") return formatNums(num2.num, decimals, false);
        		...
        		for (var i="2"; isLessThan(i, num2); i=add({num:i.split(""),isNeg:false,decimals:0}, {num:["1"],isNeg:false,decimals:0})) final = multi(final, num1);
				return final;
            }
...
var a=add, s=sub, m=multi, d=div, e=expo, l=isLessThan, g=isGreaterThan, le=isLessThanEqual, ge=isGreaterThanEqual;
